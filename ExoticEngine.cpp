...
ExoticEngine::ExoticEngine(const Wrapper<PathDependent>& TheProduct_, const Parameters& r_) :
	TheProduct(TheProduct_), r(r_), Discounts(TheProduct_->PossibleCashFlows())
{
	for (unsigned long i = 0; i < Discounts.getSize(); i++)
		Discounts.setValue(i, exp(-r.Integral(0.0, Discounts[i])));
...
{
	MJArray SpotValues(TheProduct->GetLookAtTimes().getSize());
